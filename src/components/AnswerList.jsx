...
function AnswerList({
    ...
    const revealedCorrectOption =
        hasSubmitted && isCorrectOption && !isOptionCorrect;

    function setBorderStyles() {
        if (!hasSubmitted) {
            return isSelected ? "border-3 border-vivid-violet" : "";
        }
        if (isCorrectOption && isOptionCorrect)
            return "border-3 border-spring-green";
        if (isSelected && !isOptionCorrect) return "border-3 border-soft-red";
        if (revealedCorrectOption) return "";
    }
    ...
    function setLetterStyles() {
        ...
        if (isCorrectOption && isOptionCorrect)
            return " bg-spring-green text-white";
    ...
    return (
        <li
          ...
          `}
        >
            <label className="flex justify-between items-center gap-4 w-full md:gap-8">
